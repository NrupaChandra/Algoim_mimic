...
int main(int argc, char *argv[]) {
	...
	if (argc == 4) {
		QUAD_ORDER = atoi(argv[1]);
		filename = argv[2];
		io_enabled = (strcmp(argv[3], "0") == 0 || strcmp(argv[3], "false") == 0) ? 0 : 1;
	}else if (argc == 3){
		QUAD_ORDER = atoi(argv[1]);
		filename = argv[2];
	}else if(argc == 2){
    ...
    char **ids_in = NULL;
    int *numbers_in = NULL;
    int *in_out_map = NULL; // maps from line number in input line number in output.

    char **ids_out = NULL;
    int *numbers_out = NULL;
